...
describe('normalizeMobileEs / LandlineEs / TelephoneEs', () => {
  it('strips spaces, hyphens and Spanish country prefix (+34, 0034, 34)', () => {
    expect(normalizeMobileEs('+34 611 22 33 44')).toBe('611223344');
    expect(normalizeMobileEs('0034-611-223-344')).toBe('611223344');
    expect(normalizeMobileEs('34 611 22 33 44')).toBe('611223344');
    expect(normalizeMobileEs('611223344')).toBe('611223344');
  });
  it('landline + telephone use the same rules', () => {
    expect(normalizeLandlineEs('+34 912 34 56 78')).toBe('912345678');
    expect(normalizeTelephoneEs('+34 611 22 33 44')).toBe('611223344');
  });
});
...
describe('normalize(name) dispatcher', () => {
  it('returns the right function for every alias', () => {
    ...
    expect(normalize('movil')(' +34 611 22 33 44 ')).toBe('611223344');
    expect(normalize('mobile')(' +34 611 22 33 44 ')).toBe('611223344');
...
describe('normalize → validate composition', () => {
  it('messy input becomes valid after normalize', async () => {
    ...
    expect(mobileEs(normalizeMobileEs('+34 611 22 33 44'))).toBe(true);
